...
def _follow_links(primary_capsule: dict[str, Any]) -> list[dict[str, Any]]:
    linked_capsules: list[dict[str, Any]] = []
    queue: deque[tuple[str, int]] = deque()
    visited = {primary_capsule.get("capsule_id")}

    for related_id in primary_capsule.get("related_capsule_ids", []):
        queue.append((related_id, 1))

    while queue and len(linked_capsules) < MAX_LINKED_CAPSULES:
        ...
        if payload:
            linked_capsules.append(payload)
            visited.add(capsule_id)
            for related_id in payload.get("related_capsule_ids", []):
...
def build_context_package(search_hits: dict[str, list[CapsuleHit]]) -> ContextPackage:
    ...
    score_values.extend(hit.score for hit in analytical_hits[1 : 1 + len(linked_capsules)])
